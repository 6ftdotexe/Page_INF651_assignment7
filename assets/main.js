...
function makeItalic() {
  // first select all the p elements with class as 'even'
  const paras = document.querySelectorAll('.even');
  // iterate a loop for each p element in paras
  paras.forEach( (para) => {
      // make font-style as italic
      para.style.fontStyle = "italic";
  });
  // return the nodelist of all paragraphs
  return paras;
}
...
function createBorders(){
  // the table inside the "myPage" section. 
  const myPageElem = document.querySelector("section");
...
function changeBgColor(color){
  // changes the background color as color
  myPageElem.style.backgroundColor = `${color}`;
  //returns myPage section element
